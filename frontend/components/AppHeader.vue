<template>
  <v-app-bar app color="primary" :dark="$vuetify.theme.global.name === 'dark'" height="64">
    <v-toolbar-title class="text-h6" aria-label="Логотип Practice">
      <span style="color: #FFFFFF;">Practice</span>
    </v-toolbar-title>
    <v-spacer />
    <v-btn icon to="/search" aria-label="Поиск курсов">
      <v-icon :color="$vuetify.theme.global.name === 'dark' ? '#A5D6A7' : '#28A745'">mdi-magnify</v-icon>
    </v-btn>
    ...
    >
      <v-badge :content="chatStore.unreadCount" color="error" overlap v-if="chatStore.unreadCount > 0">
        <v-icon :color="$vuetify.theme.global.name === 'dark' ? '#A5D6A7' : '#28A745'">mdi-message-text</v-icon>
      </v-badge>
      <v-icon :color="$vuetify.theme.global.name === 'dark' ? '#A5D6A7' : '#28A745'" v-else>mdi-message-text</v-icon>
    </v-btn>
  </v-app-bar>
</template>
